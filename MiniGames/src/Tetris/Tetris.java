...
public class Tetris extends JFrame implements Runnable{
	...
	public void init(){ // 기본 실행 메소드
		...
		JPanel panel = new JPanel(){
			public void paintComponent(Graphics g){
				...
				g.drawString("시작 버튼 : A", 30, 320);
				g.drawString("방향키", 30, 340);
				g.drawString(" ←  ↓  →", 30, 360);
				g.drawString("↑ : 블록 회전", 30, 380);
				g.drawString("Space : 밑으로 떨어지기 ", 30, 400);
				g.drawImage(com.getImage(),110,650,null);
			}
		};
		p3.add(panel);
	...
	void speed(){
		if(space==false){
			if (score < 11){
				speed= 250;
				if(flag2==false) System.out.println("*****현재 속도 : "+speed+" *****");
				flag2=true;
			}
			else if (score < 71){
				speed= 200;
				if(flag2==true) System.out.println("*****현재 속도 : "+speed+" *****");
				flag2=false;
			}
			else if (score < 201){
				speed= 150;
	...
	void reset(){
		...
		flag=false; flag2=false; start=false;
	}
