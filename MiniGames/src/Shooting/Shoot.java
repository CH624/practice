...
public class Shoot extends JFrame implements Runnable, KeyListener {
 ...
 public void crashChk() { //충돌 판정 메소드
  ...
  for(int i = 0; i < msList.size(); i++) {
   ...
   for(int j = 0; j < enList.size(); j++) {
    ...
    if(pol.intersects((double)e.x, (double)e.y, (double)e.w, (double)e.h)) {//적군과 미사일이 충돌했을때
     ...
     addParticle(e.x, e.y, true);addParticle(e.x, e.y, false);addParticle(e.x, e.y, true);
     addParticle(e.x, e.y, false);addParticle(e.x, e.y, true);addParticle(e.x, e.y, false);
     try {
     msList.remove(i);
  ...
  for(int i = 0; i < enList.size(); i++) {
   ...
   if(pol.intersects((double)e.x, (double)e.y, (double)e.w, (double)e.h)) {//적군과 캐릭터가 충돌했을때
	...
	addParticle(x,y,true);addParticle(x,y,false);addParticle(x,y,true);
	addParticle(x,y,false);addParticle(x,y,true);addParticle(x,y,false);
    start = false;
    end = true;
   }
  }
 ...
 public void addParticle(int x, int y, boolean bool) {
     ...
     //particles.add(new Particle(x,y,dx,dy,size,life,Color.orange,Color.pink,Color.RED));
     }
  ...
  do{
    do{
    	gs = (Graphics) bufferstrat.getDrawGraphics();
           ...
           gs.drawString("SCORE : " + score, 260, 50);
           gs.drawString("LIFE : " + life, 260, 65);
